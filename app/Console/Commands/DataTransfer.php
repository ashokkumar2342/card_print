...
class DataTransfer extends Command
{
    ...
    {  
        ...
        $datas = DB::connection('sqlsrv')->select("select AC_NO, PART_NO, SECTION_NO, SlNoInPart, C_House_no, C_House_No_V1, 
            FM_Name_EN + ' ' + IsNULL(LastName_EN,'') as name_en, FM_Name_V1 + ' ' + isNULL(LastName_V1,'') as name_l, 
            RLN_Type, RLN_FM_NM_EN + ' ' + IsNULL(RLN_L_NM_EN,'') as fname_en, 
            RLN_FM_NM_V1 + ' ' + IsNULL(RLN_L_NM_V1,'') as FName_L, EPIC_No, GENDER, AGE, 
            convert(varchar(20),DOB,105) as vDOB, DOB, EMAIL_ID, MOBILE_NO 
            from eroll where update_now = 1 and FORM_TYPE = 'form6' and last_updated_on >= '2020-11-15'");
