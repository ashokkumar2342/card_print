...
class SqlImageTransfer extends Command
{
    ...
    { 
        ...
        $ACPARTS = DB::connection('sqlsrv')->select("Select Distinct Ac_No, Part_No from eroll where update_now = 1 and FORM_TYPE = 'form6' and last_updated_on >= '2020-11-15' Order By AC_NO, Part_No");
        ...
        foreach ($ACPARTS as $key => $ac_part) {
            ...
            $datas = DB::connection('sqlsrv')->select("select SlNoInPart, PHOTO from eroll where ac_no =$ac_part->Ac_No and part_no =$ac_part->Part_No and FORM_TYPE = 'form6' and last_updated_on >= '2020-11-15' order by SlNoInPart");
